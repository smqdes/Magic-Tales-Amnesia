...
musique_in_game=pygame.mixer.Sound("In_game.wav")
musique_ecran_titre=pygame.mixer.Sound("ecran_titre.wav") #enfin on charge les musiques du jeu

continuer=True
musique_ecran_titre.play()
while continuer==True :
    fond=pygame.image.load("Ecran_titre.png")
...
continuer=True
while continuer==True :
    fond=pygame.image.load("Ecran_intro.png")
...
musique_in_game.play(loops=10) #on lance la musique avant les boucles pour qu'elle soit jouéée quelle que soit la boucle en cours. "loop" permet de rejouer la musique quand elle arrive à la fin
#1ère grande boucle
...
if pv_perso==0:
    ...
    continuer=True
    while continuer==True :
        fond=pygame.image.load("Ecran_game_over.png")
        for event in pygame.event.get():
            if event.type == QUIT:
                continuer=False
    ...
    continuer=True
    while continuer==True :
        fond=pygame.image.load("Ecran_victoire.png")
        for event in pygame.event.get():
            if event.type == QUIT:
                continuer=False
